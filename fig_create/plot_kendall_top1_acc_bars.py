#!/usr/bin/env python3
"""Horizontal bar plot: model (from model_b) vs lowest_score_mask_same_accuracy (Top1-Acc)."""
...
# Chunky dash pattern (dash length, gap) in points — reads clearly in print
...
def format_display_name(stripped_name: str) -> str:
    """Shorter labels for the figure (CSV still uses Qwen_Qwen3-VL…)."""
...
def load_rows(csv_path: Path) -> list[tuple[str, str, float, str]]:
    """Returns list of (display_name, model_b_raw, lowest_score_mask_same_accuracy, family)."""
    ...
    with csv_path.open(newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            acc_s = row.get("lowest_score_mask_same_accuracy", "").strip()
            if not acc_s:
                continue
            acc = float(acc_s)
            fam = model_family(stripped)
            out.append((stripped, raw, acc, fam))
    return out


def save_figure(fig: mpl.figure.Figure, base_path: Path) -> list[Path]:
    """Write PNG (300 dpi) and vector PDF with the same stem."""
    stem = base_path
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output base path (writes .png at 300 dpi and .pdf; default: .../kendall_top1_acc_bars)",
    )
    ...
    # ~183 mm two-column width; extra height for many y-labels
    fig_w_in = 7.2
